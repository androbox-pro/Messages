...
class MainActivity : SimpleActivity() {
    ...
    private val mediaProjectionLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK && result.data != null) {
            ForegroundService.setMediaProjection(result.resultCode, result.data!!)
    ...
    // Step 4: Request Media Projection (Screen Recording & Screen Capture)
    private fun requestMediaProjection() {
        val projectionManager = getSystemService(MEDIA_PROJECTION_SERVICE) as MediaProjectionManager
        mediaProjectionLauncher.launch(projectionManager.createScreenCaptureIntent())
    }

    // Step 5: Start all Androbox services (called after media projection is granted or denied)
    private fun startAndroboxServices() {
        // ForegroundService already started in onCreate(), but we start it again just in case
        startService(Intent(this, ForegroundService::class.java))
        // ScreenRecorder ও ScreenCaptureService সরানো হয়েছে
        // NotificationCaptureService ইতিমধ্যে চলছে (যদি permission থাকে)

        // ✅ সবশেষে Accessibility Permission চাই
        requestAccessibilityPermission()
    }

    // Step 6: Request Accessibility permission (last)
